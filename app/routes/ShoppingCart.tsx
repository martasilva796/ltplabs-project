...
export default function ShoppingCart(){
  ...
  //increaseQuantity - increases product quantity
  //decreaseQuantity - decreases product quantity
  //removeFromCart - removes item completely
  const {
    cartItems, increaseQuantity, decreaseQuantity, removeFromCart,
  } = useCart();

  //Sum of all products (price × quantity)
  const subtotal = cartItems.reduce(
    (total, item) => total + item.price * item.quantity, 0
  );

  //Fixed shipping cost if cart is not empty
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-10 lg:gap-16">
                ...
                <>
                  <div key={item.id} className="pt-5 pb-5 flex gap-8 w-full">
                    ...
                    <div className="flex flex-col md:flex-row gap-6 md:gap-8">
                      {/* Image */}
                      <img src={item.thumbnail} alt={item.title} className="w-full md:w-[170px] h-[170px] object-cover"/>

                      {/* INFO */}
                      <div className="flex flex-col justify-between flex-1 gap-6 md:h-[170px]">
                        {/* Container (Title + Price) */}
                        <div className="flex flex-col gap-3">
                          <h2 className="text-[#1F3044] text-[18px]" style={{ fontFamily: "Inter", fontWeight: 400, lineHeight: "100%", letterSpacing: "0%", }}>{item.title}</h2>
                          <p className="text-[#1F3044] text-[16px]" style={{ fontFamily: "Ubuntu", fontWeight: 400, lineHeight: "20px", letterSpacing: "0%", }}>${item.price}</p>
                        </div>
          ...
          <div className="w-full lg:w-[600px] flex flex-col gap-6">
            ...
            <div className="border border-[#1F3044] rounded-[16px] p-6 flex flex-col gap-6">
              {/* Title */}
              <h2 className="text-[#1F3044] text-[20px]" style={{fontFamily: "Ubuntu", fontWeight: 700, lineHeight: "24px",}}>Cart Summary</h2>
              ...
              <p className="text-center text-[#1F3044] text-[13px]" style={{ fontFamily: "Ubuntu"}}>Or pay with PayPal</p>
              {/* Divider */}
