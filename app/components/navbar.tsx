...
//Different navbar links for Home and Product pages
const homeLinks = ["Home", "Shop", "About", "Contact", "Blog"];
const productLinks = ["Home", "Shop", "Deals", "Contact", "Account"];

//Navbar renders different links based on the page variant
export default function Navbar({variant="home"}:{variant?:"home"|"product"}){

    //Search modal state and search data
    const [searchOpen, setSearchOpen] = useState(false);
    const [searchQuery, setSearchQuery] = useState("");
    const [results, setResults] = useState([]);
    const [loading, setLoading] = useState(false);

    //Fetch products from API when user types in search input
    useEffect(() => {
        if(!searchQuery){
            setResults([]);
            return;
        ...
        //Delay request to avoid excessive API calls while typing
        const delay = setTimeout(() => {
            ...
            fetch(`https://dummyjson.com/products/search?q=${searchQuery}`)
                ...
                .then(data => {
                    ...
                    //Prioritize products whose titles start with the search query
    ...
    //Close search modal when pressing Escape key (esc)
    useEffect(() => {
    ...
    //Prevent page scrolling while search modal is open
    useEffect(() => {
        if(searchOpen){
    ...
    //Close mobile menu when pressing Escape key (esc)
    useEffect(() => {
        function handleKeyDown(e: KeyboardEvent){
            if(e.key === "Escape"){
                setIsMenuOpen(false);
    ...
    return (
        <>
                ...
                <div className="flex items-center justify-between px-8 py-4">
                    ...
                    <div className="flex items-center">
                        <span className="text-[#1F3044] whitespace-nowrap text-[24px] lg:text-[18px] lg:text-[22px]"
                            style={{fontFamily: "Bebas Neue", lineHeight: "1", letterSpacing: "0.06em",}}>THE ONLINE STORE
                        </span>
                    </div>
                    ...
                    <div className="hidden lg:flex flex-1 justify-center">
                        <div className="flex items-center gap-8 whitespace-nowrap">
                            {(variant === "product" ? productLinks : homeLinks).map((item) => (
                                <Link key={item} to="/" className="flex items-center text-[#1F3044] transition-all duration-200 hover:opacity-70 hover:translate-y-[-1px]" style={{fontFamily: "Inter", fontSize: "15px", lineHeight: "20px",}}>
                                    <span>{item}</span>
                                    <ChevronDown className="w-4 h-4 opacity-0"/> {/* Invisible chevron-down for Figma layout consistency */}
                                </Link>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="hidden lg:flex items-center gap-6">
                            ...
                            <Link to="/ShoppingCart" className="relative w-6 h-6 flex items-center justify-center">
                                ...
                                {count > 0 && (
                                    <span className="absolute -top-2 -right-2 translate-x-[2px] translate-y-[-2px] bg-[#1F3044] text-white text-[10px] w-4 h-4 flex items-center justify-center rounded-full font-medium">{count}</span>
                                )}
                            </Link>
                        </div>
            ...
            {searchOpen && (
                <div className="fixed inset-0 z-50 bg-white/80 backdrop-blur-md overflow-y-auto flex flex-col items-center pt-32 px-6">
                    ...
                    <div className="w-full max-w-2xl">
                        <input autoFocus value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search products..." className="w-full border border-[#1F3044] rounded-lg px-4 py-3 text-sm text-[#1F3044]"/>
                    </div>
            ...
            <div className={`fixed inset-0 z-50 transition-all duration-300 ${isMenuOpen ? "pointer-events-auto" : "pointer-events-none"}`}>
                ...
                {/* Sidebar Panel */}
                <div className={`absolute right-0 top-0 h-full w-72 bg-white shadow-lg p-6 transform transition-transform duration-300 ${ isMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
                    {/* Close Button */}
